...
void Lab6::Update(float deltaTimeSeconds)
{
...

	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 1, 0));
		RenderVariableMesh(meshes["box"], shaders["LabShader"], modelMatrix, Engine::GetElapsedTime());
	}
...

	{
		...
		modelMatrix = glm::rotate(modelMatrix, RADIANS(60.0f), glm::vec3(1, 0, 0));
		modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
		RenderVariableMesh(meshes["cube"], shaders["LabShader"], modelMatrix, Engine::GetElapsedTime());
	}
}
...
void Lab6::RenderVariableMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, GLfloat deltaTime)
{
	if (!mesh || !shader || !shader->GetProgramID())
		return;
	...
	GLint locTimeVec = glGetUniformLocation(shader->program, "TimeVec");
	//glm::vec4 timeVector = glm::vec4(
	//	std::cos(deltaTime) * 2,
	//	std::sin(deltaTime) * 2,
	//	std::cos(deltaTime) + std::sin(deltaTime),
	//	1
	//);
	glm::vec4 timeVector = glm::vec4(
		std::cos(deltaTime),
		std::cos(deltaTime + AI_MATH_HALF_PI),
		std::cos(deltaTime - AI_MATH_HALF_PI),
		1
	);
	glUniform4fv(locTimeVec, 1, glm::value_ptr(timeVector));

	glBindVertexArray(mesh->GetBuffers()->m_VAO);
	glDrawElements(mesh->GetDrawMode(), static_cast<int>(mesh->indices.size()), GL_UNSIGNED_INT, 0);

	glUniform4fv(locTimeVec, 1, glm::value_ptr(glm::vec4(1)));
}
