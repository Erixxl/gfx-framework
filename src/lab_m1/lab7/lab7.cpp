...
void Lab7::Init()
{
	...
	// Light & material properties
	{
		...
		planeColor = glm::vec3(0.5);
		sphereColor = glm::vec3(0.5);
	}
}
...
{
	{
		...
		RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, sphereColor);
	}
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, planeColor);
	}
...
void Lab7::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_1)
	{
		r = (rand() % 256) * 1.0f / 255.0f;
		g = (rand() % 256) * 1.0f / 255.0f;
		b = (rand() % 256) * 1.0f / 255.0f;

		planeColor = glm::vec3(r, g, b);
	}
